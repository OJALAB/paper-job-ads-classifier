...
```{r}
multilang <- py_load_object("../results/NAWA-trans-multi-overall.pickle", convert = T)
...
multilang[measure == "recall@1" & type == "test" & dataset %in% c("overall", "cbop", "hand")][
...
```{r}
prepare_sorted_data <- function(data, x_col, y_col, facet_col) {
  # Get the ordering from the first facet
  order_df <- data %>%
    filter(!!sym(facet_col) == first(!!sym(facet_col))) %>%
    arrange(desc(!!sym(y_col))) %>%
    select(!!sym(x_col))
  
  # Convert x column to factor with levels based on the order
  data %>%
    mutate(!!sym(x_col) := factor(!!sym(x_col), 
                                 levels = order_df[[x_col]]))
}
```
...
multilang_for_plot |>
  ...
  transform(flag = `top-down` > `bottom-up`,
            diff = `top-down` - `bottom-up`) |> 
  #subset(variable == "4 digits\nISCO") |>
  #summary()
...
writeData(wb = excel_results, 
          x=rbind(linear_results_df, transformer_results_df, transformer_large_results_df), 
          sheet = "Polish-lang-only",
          withFilter = T)

addWorksheet(excel_results, "Bilingual")
writeData(wb = excel_results, 
          x=rbind(multi_polish[, trans:="Google"], multi_english[, trans:="Google"], 
                multi_polish_arg[, trans:="Argos"], multi_english_arg[, trans:="Argos"],
                multi_english_her[, trans:=""],
                transformer_results_df[model %in% c("Transformer(bottom)", "Transformer(top)")][, trans:=""][, language:="Polish"]), 
          sheet = "Bilingual",
...
writeData(wb = excel_results, 
          x=multilang, sheet = "Multilingual",
          withFilter = T)

saveWorkbook(excel_results, file = "../results/Supplementary-Materials.xlsx", overwrite = T)
```
