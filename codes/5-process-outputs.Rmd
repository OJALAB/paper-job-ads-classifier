...
```{r}
multi_polish_arg <- py_load_object("../results/NAWA-trans-multi-pl-results.pickle", convert = T)
...
multi_english_arg <- py_load_object("../results/NAWA-trans-multi-en-results.pickle", convert = T)
  ...
  transform(variable = factor(variable, paste0("level", 0:4), 
            ...
            model = factor(model, 
                           ...
                           labels = c("Polish Transformer (XLM-RoBERTa-base), bottom-up (Google)", 
                                      ...
                                      "English Transformer (HerBERT-base), top-down"))) -> data_for_plot
...
ggplot(data = data_for_plot, aes(x = variable, 
  ...
  theme(legend.position = c(1, 0.5), legend.justification = c(1.25, 1.25)) +
  coord_flip() + 
  guides(fill = guide_legend(reverse = TRUE)) -> p2
